...
const FormLegalitas = () => {
  ...
  useEffect(() => {
    if (initialLoad) {
      let retrievedObject = JSON.parse(localStorage.getItem('skoringLegal'));
  ...
  const storeData = preventDefault(async () => {
    localStorage.setItem("skoringLegal", JSON.stringify(dataSubmit));
    ...
    const res = axios.post(
      `${appConfig.baseUrl}/evaluations/${localStorage.getItem('evaluationId')}/scorings/${localStorage.getItem('scoringId')}/legalities`,
      {
        score: data
      }
    );
    ...
    res.then(
      function(dt) {
        ...
        if (dt.data.status == 'success') {
          router.push({
            pathname: "/admin/master-basis-data/manajemen"
          })
        }
    ...
    <>
      <form>
        ...
        <div className={`${mng["base__formsection"]}`}>
          <span className={mng.base__subtitle}>Kelompok A: Izin untuk berusaha di bidang perkebunan</span>
          <p className={`${mng["base__formtitle"]} ${'mt-4'}`}>Subkelompok 1: Kepemilikan izin</p>
