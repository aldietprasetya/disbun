...
const SurveyHarianDetailPage = ({ dtwControlId }) => {
  const router = useRouter();
  const [data, setData] = useState(null);

  const getDailySurveyData = useCallback(async () => {
    try {
      const res = await axiosInstance.get(
        `/dtw-control/v1/daily-survey/${dtwControlId}`,
      );
      ...
      <div className="h-full rounded bg-white py-5 px-7">
        ...
        <div className="mt-7 flex w-full justify-center">
          <div className="flex w-[60%] flex-col items-center ">
            ...
            <SurveyHarianForm data={data} type="detail" />
          </div>
        </div>
      </div>
    </Page>
...
export default SurveyHarianDetailPage;
